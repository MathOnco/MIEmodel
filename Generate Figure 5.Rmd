...
3) Set up 'departureFromHomeo' data frame of 1-death/birth where death=1-growth/birth, and replace negative entries with 0
...
6) Add 1-death/birth to 'departureFromHomeo' data frame
7) Set x and y-labels for box plots
```{r}
  ...
  data.frame(
    sapply(birth_and_growth_Rates$type, function(x)
      rlnorm(N_SAMPLES, meanlog = birth_and_growth_Rates[birth_and_growth_Rates$type ==
                                                           x, ]$GrowthRateMean_lognormDist, sdlog = birth_and_growth_Rates[birth_and_growth_Rates$type == x, ]$GrowthRateSD_lognormDist)),
    check.names = FALSE,
...
bRLN <-
  data.frame(
    sapply(birth_and_growth_Rates$type, function(x)
      rlnorm(N_SAMPLES, meanlog = birth_and_growth_Rates[birth_and_growth_Rates$type ==
                                                           x, ]$BirthRateMean_lognormDist, sdlog = birth_and_growth_Rates[birth_and_growth_Rates$type == x, ]$BirthRateSD_lognormDist)),
    check.names = FALSE,
    fix.empty.names = FALSE
  )
...
departureFromHomeo <-
  data.frame(sapply(colnames(bRLN), function(x)
    ...
    check.names = FALSE,
    fix.empty.names = FALSE)
departureFromHomeo[departureFromHomeo > 1] <- 1
departureFromHomeo <- tidyr::gather(departureFromHomeo, key = "type", value = 'one_minus_d_over_b')
...
## set x and y labels for box plots
...
```{r}
  plot.x <-
    ggplot(departureFromHomeo) + geom_boxplot(aes(type, one_minus_d_over_b))  + coord_flip()
plot.y <-
...
df$category <- sort(unique(departureFromHomeo$type))
```
